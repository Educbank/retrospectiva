...
// TestTeamSchema validates the Team model structure
func TestTeamSchema(t *testing.T) {
	tests := []struct {
		name     string
		team     models.Team
		validate func(t *testing.T, original, unmarshaled models.Team)
	}{
		{
			name: "Complete Team with description",
			team: models.Team{
				ID:          uuid.New(),
				Name:        "Test Team",
			...
			validate: func(t *testing.T, original, unmarshaled models.Team) {
				...
				assert.Equal(t, original.OwnerID, unmarshaled.OwnerID)
				assert.Equal(t, original.CreatedAt.Unix(), unmarshaled.CreatedAt.Unix())
				assert.Equal(t, original.UpdatedAt.Unix(), unmarshaled.UpdatedAt.Unix())
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Test JSON marshaling
			jsonData, err := json.Marshal(tt.team)
			require.NoError(t, err)

			// Test JSON unmarshaling
			var unmarshaledTeam models.Team
			err = json.Unmarshal(jsonData, &unmarshaledTeam)
			require.NoError(t, err)

			// Validate fields
			tt.validate(t, tt.team, unmarshaledTeam)
		})
	}
}

// TestTeamCreateRequestSchema validates the TeamCreateRequest structure
func TestTeamCreateRequestSchema(t *testing.T) {
	...
	// Test JSON unmarshaling
	var unmarshaledReq models.TeamCreateRequest
	err = json.Unmarshal(jsonData, &unmarshaledReq)
	require.NoError(t, err)
